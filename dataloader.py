...
# here, we use docstring commenting style: https://peps.python.org/pep-0257/
...
from monai.transforms import (
    ...
    RandSpatialCropd, RandRotate90d, RandAffined, ResizeWithPadOrCropd, RandGridDistortiond, Rand3DElasticd
)
...
# classes of TotalSegmentator_v201 dataset:
total = {
        1: "spleen",
        2: "kidney_right",
        3: "kidney_left",
...
class TotalSeg_Dataset_Tr_Val(Dataset):
    ...
    def combine_masks(self, lbl_dir):
        """ 
        Method for combining individual labels into a single multiple-class mask.
        """
        out_path = os.path.join(lbl_dir, 'combined_mask.nii.gz')  # source: https://docs.python.org/3/library/os.path.html
        ...
        if not os.path.exists(out_path):  # check if the combined mask already exists
            ...
            for class_id, organ_name in total.items():
                ...
                if os.path.exists(struct_path):
                    mask = nib.load(struct_path).get_fdata() # source: https://nipy.org/nibabel/images_and_memory.html - load mask data
                    if combined_mask is None:
                        combined_mask = np.zeros_like(mask) # source: https://numpy.org/doc/stable/reference/generated/numpy.zeros_like.html - generating empty mask
            ...
            if combined_mask is not None:
                ...
                combined_mask = np.zeros((128,128,128), dtype=np.uint8) # source: https://numpy.org/doc/2.1/reference/generated/numpy.zeros.html
...
def batch_collate_fn_tr_val(batch):  # here we called arg 'batch' instead of data
    ...
    source #1: https://lukesalamone.github.io/posts/custom-pytorch-collate/
    source #2:https://pytorch.org/docs/stable/_modules/torch/utils/data/_utils/collate.html#default_collate
    """
...
"""
    Method that loads and preapres data for training, validation and testing.
...
def get_dataloaders(base_dir, meta_csv, combine_masks = True, batch_size = 1, num_workers = 1):
        """Helper method to get paths for imgs and their corresponding lbl directories."""
        ...
        # loading meta.csv file to determine train/test/val split:
        try:
