...
def parse_and_calculate_iou(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        # Map organ names to their indices in the header for Dice columns
        ...
        for row in reader:
            for organ, idx in organ_indices.items():
                try:
                    ...
                    # Calculate IoU using the formula: IoU = Dice / (2 - Dice)
...
def plot_organ_iou(organ_iou):
    ...
    # Calculate mean IoU for each organ
    ...
    # Organ names and their respective mean IoU scores
    ...
    plt.gca().invert_yaxis()  # Invert the y-axis to show the highest IoU at the top
