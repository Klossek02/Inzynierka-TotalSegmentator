...
# converting NifTi file into saggital, coronal and axial 2D slices, resizing them to a target size (the one fitting to the window):
...
# displaying a single slice within provided QLabel and QPixmap (contianing this slice):
...
# function to convert segmentation slices into sagittal, coronal and axial ones:
...
# function to convert segmentation mask (2D) to QPixmap:
...
# function to overlay segmentation slices on provided (base) images with specified transparency: 
def overlay_slices(base_slice: QPixmap, mask_slice: QPixmap,
    ...
    # Create overlay BGR - fill pixels based on label
...
# function to convert hex color to BGR (Blue, Green, Red) format:
...
# function to obtain scan orientation using its affine matrix: 
def get_scan_orientation(affine):
    return nib.orientations.io_orientation(affine)


# function to reorient scan to match the desired orientation: 
